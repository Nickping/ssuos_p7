...
fs_mount(struct ssu_fs *fs, struct blk_dev *dev) {
	...
	/* Load or Initialize Block bitmap & Inode bitmap Load or Initialize */
	/* Modify and add your code */

	//load BITMAP_BLOCK from HDD
	result = fs_readblock(fs, SSU_BITMAP_BLOCK, bitmapblock);

	//load or initailize block bitmap
	fs->fs_blkmap = (struct bitmap *) bitmapblock;
	if (fs->fs_blkmap->bit_cnt != fs->fs_sb.sb_nblocks) // Condition can be modified
	{
		bitmap_create_in_buf(fs->fs_sb.sb_nblocks,fs->fs_blkmap,2048);
		printk("initialize block bitmap : %d bit\n", fs->fs_blkmap->bit_cnt);
	} else
		printk("load block bitmap : %d bit\n", fs->fs_blkmap->bit_cnt);

	//load or initailize inode bitmap
	fs->fs_inodemap = (struct bitmap *) (bitmapblock + SSU_BLOCK_SIZE / 2);
	if (fs->fs_inodemap->bit_cnt != NUM_INODE) // Condition can be modified
	{
		bitmap_create_in_buf(NUM_INODE,fs->fs_inodemap,2048);
...
int fs_readblock(struct ssu_fs *fs, uint32_t blknum, char *buf) {
	...
	for (i = 0; i < 8; i++)
		DEVOP_READ(fs->fs_device, startsec + i/* + ? */, buf + i * 512/* + ? */);
...
int fs_writeblock(struct ssu_fs *fs, uint32_t blknum, const char *buf) {
	...
	int startsec = blknum*8;
	for (i = 0; i < 8; i++)
		DEVOP_WRITE(fs->fs_device, startsec + i/* + ? */, buf + i * 512/* + ? */);
